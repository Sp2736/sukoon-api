...
export const propertySchema = z.object({
    ...
    category: z.enum(['Residential', 'Commercial', 'Agricultural Land', 'Non-agricultural Land'], {
    ...
    related_properties: z.preprocess(
        (val) => {
            if (!val) return [];
            if (typeof val === 'string') return [val];
            return val;
        },
        z.array(z.string()).max(3, "You can select a maximum of 3 related properties")
    ).optional(),
